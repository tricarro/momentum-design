import{S as v,U as y,R as f,k as l,V as x,v as E,T as o,C as b,L as u,n as s}from"./iframe-B68zFh-R.js";import{D as L}from"./DisabledMixin-Cqnt9CUs.js";import{K as k,a as I,T as S,A as d}from"./KeyDownHandledMixin-BiZdIgE3.js";import{a as D}from"./LifeCycleMixin-CkFTW4AQ.js";import{D as w}from"./listitem.constants-CKsHLKpD.js";class g{static dispatchListItemEvent(t,e){e.dispatchEvent(new Event(t,{composed:!0,bubbles:!0}))}static onDisableListItem(t){this.dispatchListItemEvent("disabled",t)}static onEnableListItem(t){this.dispatchListItemEvent("enabled",t)}static onCreatedListItem(t){setTimeout(()=>{this.dispatchListItemEvent("created",t)},0)}static onDestroyedListItem(t){this.dispatchListItemEvent("destroyed",t)}}const C=v`
  :host {
...
`,T=[C,...y()];var A=Object.defineProperty,a=(n,t,e,c)=>{for(var r=void 0,h=n.length-1,m;h>=0;h--)(m=n[h])&&(r=m(t,e,r)||r);return r&&A(t,e,r),r};const p=class p extends k(I(L(S(D(b))))){constructor(){super(),this.variant=w.VARIANT,this.wasSpacePressed=!1,this.addEventListener("keydown",this.handleKeyDown.bind(this)),this.addEventListener("keyup",this.handleKeyUp.bind(this)),this.addEventListener("click",this.handleClick.bind(this)),this.addEventListener("blur",this.handleBlur.bind(this))}connectedCallback(){super.connectedCallback(),this.role=this.role||f.LISTITEM}handleBlur(){this.wasSpacePressed=!1}handleClick(t){this.disabled&&(t.stopImmediatePropagation(),t.preventDefault())}handleKeyDown(t){const e=this.getActionForKeyEvent(t);this.isEventFromInsideListItem(t)||(!t.defaultPrevented&&e===d.ENTER?(this.keyDownEventHandled(),t.preventDefault(),this.triggerClickEvent(t)):e===d.SPACE&&(this.wasSpacePressed=!0))}handleKeyUp(t){this.getActionForKeyEvent(t)===d.SPACE&&(!this.isEventFromInsideListItem(t)&&!t.defaultPrevented&&this.wasSpacePressed&&(t.preventDefault(),this.triggerClickEvent(t)),this.wasSpacePressed=!1)}isEventFromInsideListItem(t){const e=t.target;return e!==this&&document.activeElement===e&&this.contains(e)}triggerClickEvent(t){if(this.isEventFromInsideListItem(t))return;const e=new MouseEvent("click",{bubbles:!0,cancelable:!0,view:window});this.dispatchEvent(e)}getText(t,e,c){return l`
      <slot name="${t}">
        ${c?l`<mdc-text part="${t}" type="${e}" tagname="${x.SPAN}">${c}</mdc-text>`:E}
      </slot>
    `}disableSlottedChildren(t=!1){[...this.leadingControlsSlot,...this.trailingControlsSlot].forEach(e=>{t?(e.setAttribute("disabled",""),g.onDisableListItem(this)):(e.removeAttribute("disabled"),g.onEnableListItem(this))}),this.setAttribute("aria-disabled",`${t}`)}update(t){super.update(t),t.has("disabled")&&(this.tabIndex=this.disabled?-1:0,this.disableSlottedChildren(this.disabled),this.dispatchModifiedEvent(this.disabled?"disabled":"enabled")),t.has("softDisabled")&&this.disableSlottedChildren(this.softDisabled),t.has("dataIndex")&&(this.ariaPosInSet=`${this.dataIndex!==void 0?this.dataIndex+1:""}`)}renderTrailingControls(){return l`<slot
    ...
    ></slot>`}renderLeadingControls(){return l`<slot
      ...
      @keyup=${this.stopEventPropagation}
      @keydown=${this.stopEventPropagation}
    ></slot>`}stopEventPropagation(t){if(t instanceof KeyboardEvent){const e=this.getActionForKeyEvent(t);(e===d.ENTER||e===d.SPACE)&&t.stopPropagation()}else t instanceof MouseEvent&&t.stopPropagation()}click(){this.triggerClickEvent(new Event("click"))}render(){return l`
      <slot name="content">
        <div part="leading">
